function res = matconvnet_simplenn(net, x, dzdy, res, varargin)
%% Modified by Xinchen Yan, Nov. 26, 2014
...
for i=1:n
  ...
  switch l.type
    ...
    case 'bias' % TODO
      res(i+1).x = matconvnet_nnbias(res(i).x, l.biases);
    case 'reshape' % TODO
      res(i+1).x = matconvnet_nnreshape(res(i).x, l.winsize);
    case 'binloss' % TODO
      res(i+1).x = matconvnet_nnbinloss(res(i).x, l.class);
    case 'sigmoid' % TODO
      res(i+1).x = matconvnet_nnsigmoid(res(i).x);
    case 'euloss'
      res(i+1).x = matconvnet_nneuloss(res(i).x, l.class); %% TODO
    case 'pool'
...
if doder
  ...
  for i=n:-1:1
    ...
    switch l.type
      ...
      case 'unpool' % TODO
        res(i).dzdx = vl_nnunpool(res(i).x, res(i+1).dzdx, ...
          'stride', l.stride);
      case 'bias' % TODO
        [res(i).dzdx, res(i).dzdw{2}] = ...
            matconvnet_nnbias(res(i).x, l.biases, res(i+1).dzdx);
      case 'reshape' % TODO
        res(i).dzdx = matconvnet_nnreshape(res(i).x, l.winsize, res(i+1).dzdx);
      case 'binloss' % TODO
        res(i).dzdx = matconvnet_nnbinloss(res(i).x, l.class, res(i+1).dzdx);
      case 'sigmoid' % TODO
        res(i).dzdx = matconvnet_nnsigmoid(res(i).x, res(i+1).dzdx);
      case 'euloss'
        res(i).dzdx = matconvnet_nneuloss(res(i).x, l.class, res(i+1).dzdx);
      case 'pool'
