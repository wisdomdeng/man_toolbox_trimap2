function [net_enc, net_dec, net_style1, net_style2, info] = ...
  cvae2comb_train_adam(opts, net_enc, net_dec, net_style1, net_style2, ...
...
info = struct;
info = vaecomb_info_init(opts, info, 0);
...
%% TODO
net_dec.layers{end}.lambda = opts.lambda_grad;
...
for epoch=epoch_start+1:opts.numEpochs
  ...
  info = vaecomb_info_init(opts, info, 1);
  ...
  for t = 1:opts.batchSize:numel(train)
    ...
    LL = gather(res_dec(end).ll)  / opts.batchSize;
    Gradcost = gather(res_dec(end).ll - res_dec(end).x) / opts.batchSize;

    batch_time = toc(batch_time);
    speed = numel(batch)/batch_time;

    info.train = updateError(opts, info.train, KL, LL, Gradcost, batch_time);

    fprintf(' %.2f s (%.1f images/s)', batch_time, speed);
    n = t + numel(batch) -1;
    fprintf(' LB %.3f KL %.3f LL %.3f Gcost %.3f', info.train.objective(end)/n, ...
      info.train.kl(end)/n, info.train.ll(end)/n, info.train.gc(end)/n);
  ...
  for t = 1:opts.batchSize:numel(val)
    ...
    info.val = updateError(opts, info.val, KL, LL, Gradcost, batch_time);

    fprintf(' %.2f s (%.1f images/s)', batch_time, speed);
    n = t + numel(batch) - 1;
      
    fprintf(' LB %.3f KL %.3f LL %.3f Gcost %.3f', info.val.objective(end)/n, ...
      info.val.kl(end)/n, info.val.ll(end)/n, info.val.gc(end)/n);
  ...
  [info, opts] = vaecomb_drawcurve(info, opts, epoch);
