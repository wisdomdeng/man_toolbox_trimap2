function [net_enc, net_dec, info] = vae_train_adam(opts, net_enc, net_dec, imdb, getBatch, updateError)
...
for epoch=epoch_start+1:opts.numEpochs
  ...
  for t = 1:opts.batchSize:numel(train)
    ...
    [data, noise] = getBatch(imdb, batch, opts) ;
    %[cost, res_enc, res_dec] = vae_simplenn_cost('both', opts, data, noise, net_enc, net_dec, res_enc, res_dec);
    [cost, res_enc, res_dec] = vae_cost(data, noise, net_enc, net_dec, res_enc, res_dec, opts.cpt_mode);
    ii = ii + 1;
    % gradient update
    if opts.lr_enc > 0,
      net_enc = vae_param_update_adam(opts, net_enc, res_enc, opts.lr_enc*lr, beta1, beta2, epsilon, ii, 0);
    end
  ...
  for t = 1:opts.batchSize:numel(val)
    ...
    [data, noise] = getBatch(imdb, batch, opts) ;
    %[cost, res_enc, res_dec] = vae_simplenn_cost('forward', opts, data, noise, net_enc, net_dec, res_enc, res_dec);
    [cost, res_enc, res_dec] = vae_cost(data, noise, net_enc, net_dec, res_enc, res_dec);
  ...
  if mod(epoch, opts.saveround) == 0
    save(sprintf(modelPath, epoch), 'net_enc', 'net_dec', 'info');
  end
...
modelPath = fullfile(opts.expPath, 'net-epoch-%d.mat');
load(sprintf(modelPath, opts.numEpochs), 'net_enc', 'net_dec', 'info');
