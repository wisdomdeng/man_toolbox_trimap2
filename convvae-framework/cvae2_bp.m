function [res, res_style] = cvae2_bp(net, net_style, style, noise, dzdy, res, res_style, varargin)
...
for i = n:-1:1,
    ...
    switch l.type
        ...
        case 'fc_mix2_gaussian'
            ...
            [res(i).dzdx, res(i).filters, res(i).biases] = ...
                vl_nnconv(cat(3, res(i).x, res_style(end).x), l.filters, l.biases, ...
                res(i+1).dzdx, 'pad', l.pad, 'stride', l.stride);
          
            h_range = [1:size(res(i).dzdx, 3)-size(res_style(end).x, 3)];
            style_range = [h_range(end)+1:size(res(i).dzdx,3)];

            res_style(end).dzdx = res(i).dzdx(:,:,style_range, :);
            res_style = vae_bp(net_style, [], [], res_style);

            res(i).dzdx = res(i).dzdx(:,:,h_range,:);
        ...
        case 'convt_gaussian',
            ...
            % linear (convt) response for mean
            [res(i).dzdx, res(i).filters, res(i).biases] = ...
                vl_nnconvt(res(i).x, l.filters, l.biases, ...
                           res(i+1).dzdx, ...
                           'Crop', l.pad, 'Upsample', l.stride);
            
            % linear (convt) response for std
            [res(i).dzds, res(i).filters_std, res(i).biases_std] = ...
                vl_nnconvt(res(i).x, l.filters_std, l.biases_std, ...
                           res(i+1).dzds, ...
                           'Crop', l.pad, 'Upsample', l.stride);
        ...
        case 'gaussian_ll',
            ...
            if isfield(l,'lambda') && l.lambda > 0,
              ...
              v_grad = imgrad_bp(l.lambda, v_d1, v_d2, v_mu1, v_mu2, 1) / l.nsample;
              res(i).dzdx = res(i).dzdx - v_grad;
            end
        ...
        case 'euclidean_ll',
            % loss for real input (fixed diagnal sigma)
            res(i).dzdx = -res(i+1).dzdx*bsxfun(@rdivide, bsxfun(@minus, res(i).x, l.data), res(i).s.^2);
            res(i).dzdx = reshape(res(i).dzdx, size(l.data, 1), size(l.data, 2), size(l.data, 3), size(l.data, 4)*l.nsample)/l.nsample;
            ...
            avgDzds = sum(sum(sum(res(i).dzds, 1), 2), 3) / size(res(i).dzds, 1) / size(res(i).dzds, 2) / size(res(i).dzds, 3);
            res(i).dzds = bsxfun(@times, avgDzds, ones(size(res(i).dzds, 1), size(res(i).dzds, 2), size(res(i).dzds, 3), size(res(i).dzds, 4)));

        case 'imgrad_gaussian_ll'
            %% from color channel
            res(i).dzdx = -res(i+1).dzdx*bsxfun(@rdivide, bsxfun(@minus, res(i).x, l.data), res(i).s.^2);
            res(i).dzds = -0.5*(3*one - bsxfun(@rdivide, bsxfun(@minus, res(i).x, l.data), res(i).s).^2);
            %%
            [v_d1, v_d2] = vae_compute_imgrad(l.data);
            [v_mu1, v_mu2] = vae_compute_imgrad(res(i).x);
            
            [v_gradx, v_grads] = imgrad_gaussian_bp(l.lambda, v_d1, v_d2, v_mu1, v_mu2, res(i).s, 1);
            %% from gradient
            res(i).dzdx = res(i).dzdx + v_gradx / l.nsample;
            res(i).dzds = res(i).dzds + v_grads / l.nsample;
            %% reshaping
            res(i).dzdx = reshape(res(i).dzdx, size(l.data, 1), size(l.data, 2), size(l.data, 3), size(l.data, 4)*l.nsample)/l.nsample;
            res(i).dzds = reshape(res(i).dzds, size(l.data, 1), size(l.data, 2), size(l.data, 3), size(l.data, 4)*l.nsample)/l.nsample;
        ...
        case 'noise'
            if opts.disableDropout
              ...
              res(i).dzdx = vl_nnfmapnoise_bp(res(i).x, res(i+1).dzdx, res(i).n, l.rate);
            end
